...
public class SpriteRenderer extends Renderer{
    ...
    public void render(Graphics2D g) {
        ...
        Vector2 scale = transform.getGlobalScale().multiply(new Vector2(100,-100));
        int x = (int) getPos().getX()-(int)scale.getX()/2;
        int y = (int) getPos().getY()-(int)scale.getY()/2;
        ...
        if(currentSprite != null){
            if (!isInverted())
                g.drawImage(currentSprite.getImage(),x,y,(int) scale.getX(),(int) scale.getY(), null);
            else
                g.drawImage(currentSprite.getImage(),x,y,-(int) scale.getX(),(int) scale.getY(), null);
        }
        g.setColor(Color.RED);
        g.drawRect(x,-y,(int) scale.getX(),-(int) scale.getY());
